...
// =============================================================================
// Extract all the player scores from the europeantour.com webpage.
...
function extract_all_player_scores() {
	...
	// extract all the row objects from the leaderboard
	var row_objects = document.querySelectorAll(".leaderboard__row");
	
	// loop through row objects and keep only the rows that contain the
	// substring 'table__cell-inner--name' (these are the player rows)
	var sentinel_string = 'table__cell-inner--name';
	...
	for (var i = 0; i < row_objects.length; ++i) {
		...
		var row_html = row_object.innerHTML;
		// get player name
		var player_name_string = extract_substring(row_html, 
			'<span class="leaderboard__name"', '>', '</span>');
		// get surname
		var surname = extract_substring(player_name_string, '<', 'strong>',
																	'</strong>');
		surname = surname.replace(',', '');
		surname = surname.trim();
		// get first name
		var loc = player_name_string.indexOf('</strong>') + 9;
		var first_name = player_name_string.substring(loc);
		first_name = first_name.replace('*', '');
		first_name = first_name.replace('(AM)', '');
		first_name = first_name.trim();
		// build full name
		var player_name = first_name.toUpperCase() + ' ';
		player_name += surname.toUpperCase();
		// get first round score
		var first_round_score_html = extract_substring(row_html,
			'table__cell--round-1">', '<div', '</div>');
		loc = first_round_score_html.indexOf('>');
		var first_round_score = first_round_score_html.substring(loc + 1);
		first_round_score = first_round_score.trim();
		// get second round score
		var second_round_score_html = extract_substring(row_html,
			'table__cell--round-2">', '<div', '</div>');
		loc = second_round_score_html.indexOf('>');
		var second_round_score = second_round_score_html.substring(loc + 1);
		second_round_score = second_round_score.trim();
		// get third round score
		var third_round_score_html = extract_substring(row_html,
			'table__cell--round-3">', '<div', '</div>');
		loc = third_round_score_html.indexOf('>');
		var third_round_score = third_round_score_html.substring(loc + 1);
		third_round_score = third_round_score.trim();
		// get fourth round score
		var fourth_round_score_html = extract_substring(row_html,
			'table__cell--round-4">', '<div', '</div>');
		loc = fourth_round_score_html.indexOf('>');
		var fourth_round_score = fourth_round_score_html.substring(loc + 1);
		fourth_round_score =fourth_round_score.trim();
		...
		var result_object = {
					...
					"third_round_score"		: third_round_score,
					"fourth_round_score"	: fourth_round_score
								};
